...
describe('Structured Medical Data approval cache contract', () => {
  it('invalidates structured data and Consultation caches only after approval succeeds', async () => {
    ...
    queryClient.setQueryData(queryKeys.structuredData(42), { approved: false });
    queryClient.setQueryData(queryKeys.consultation(42), { id: 42 });
    ...
    expect(queryClient.getQueryState(queryKeys.structuredData(42))?.isInvalidated).toBe(false);
    expect(queryClient.getQueryState(queryKeys.consultation(42))?.isInvalidated).toBe(false);

    releaseResponse.resolve();
    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(queryClient.getQueryState(queryKeys.structuredData(42))?.isInvalidated).toBe(true);
    expect(queryClient.getQueryState(queryKeys.consultation(42))?.isInvalidated).toBe(true);
  });
});
