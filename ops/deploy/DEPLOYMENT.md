...
```bash
./scripts/release/package-release.ps1                 # tag "latest"
# or a versioned build:
./scripts/release/package-release.ps1 -Tag 2026-08-20
```

This wipes and rebuilds `release/<Tag>/` from scratch (never inherits stale files from a
previous package), builds the 5 app images (backend-api, backend-migrations,
clinical-knowledge, transcription-worker, proxy), `docker save`s them into
`release/<Tag>/images/*.tar`, and copies `docker-compose.prod.yml`, `ops/postgres/db-init/`,
the 3 rabbitmq files prod actually needs (`rabbitmq.conf`, `enabled_plugins`, `provision.sh`),
the deploy scripts, and `.env.prod.example` into `release/<Tag>/`. Before building it checks
`docker-compose.prod.yml`'s image-tagged services against the build list (fails fast on
drift); after assembling it scans for secret-shaped files and checks every file against an
explicit content allowlist, then writes `manifest.json` (tag, image prefix, git commit/branch,
build time, and each image's verified tag) — that file is what `deploy.sh` checks against
`.env.prod` before it will deploy (step 4).

Older tagged releases (`release/2026-08-20/`, etc.) are untouched siblings — keep them around
for rollback, or delete them by hand once retired.

---

## 2. Upload with FileZilla

Connect: **SFTP**, host = VM IP, port **22**, your VM user. Upload the **entire `release/<Tag>/`
folder** into `/opt/medicalassistant` on the VM (e.g. as `/opt/medicalassistant/release`, or
keep the tag in the path if you want multiple releases side by side). You should end up with
that folder containing `manifest.json`, `images/`, `docker-compose.prod.yml`,
`ops/postgres/db-init/`, `ops/rabbitmq/`, `deploy.sh`, `init-letsencrypt.sh`, `.env.prod.example`.

> Re-deploys only need to re-upload `images/` and `manifest.json` (and the compose file if it
> changed) — the config folders rarely change.
...
First, it checks `manifest.json` (if present) against `.env.prod`'s `IMAGE_TAG`/`IMAGE_PREFIX`
and refuses to proceed if they disagree — a packaged-for-the-wrong-tag bundle fails loudly
here instead of silently running the wrong images.
...
```bash
# on your machine
./scripts/release/package-release.ps1 -Tag 2026-08-21
# FileZilla: upload release/2026-08-21/images/ and manifest.json (and docker-compose.prod.yml
# if changed) into the VM's release folder, then set IMAGE_TAG=2026-08-21 in .env.prod
# on the VM
