...
function Get-ControllerRoutes {
    ...
    foreach ($controllerDirectory in $ControllerDirectories) {
        Get-ChildItem -LiteralPath $controllerDirectory -Filter "*Controller.cs" -File -Recurse | ForEach-Object {
            $controllerFile = $_
...
$eventContractSource = Get-Content -Raw -LiteralPath (Join-Path $repositoryRoot "backend/src/MedicalAssistant.ConsultationProcessing.Contracts/ConsultationIntegrationEvents.cs")
...
$envelopeSource = Get-Content -Raw -LiteralPath (Join-Path $repositoryRoot "backend/src/MedicalAssistant.EventBus/Serialization/IntegrationEventEnvelope.cs")
...
$payloadSource = Get-Content -Raw -LiteralPath (Join-Path $repositoryRoot "backend/src/MedicalAssistant.ConsultationProcessing.Contracts/ConsultationContracts.cs")
$payloadFields = @{}
